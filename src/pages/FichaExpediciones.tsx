import { useState } from "react";

const tabs = [
  { id: 0, label: "Inicio", short: "Inicio" },
  { id: 1, label: "¿Para Quién?", short: "Servicios" },
  { id: 2, label: "Cómo Funciona", short: "Proceso" },
  { id: 3, label: "Beneficios", short: "Beneficios" },
  { id: 4, label: "Contacto", short: "Contacto" },
];

// Componente de icono industrial: fondo oscuro sólido con contenido blanco
const IndustrialIcon = ({ 
  children, 
  variant = "default",
  size = "md" 
}: { 
  children: React.ReactNode; 
  variant?: "default" | "blue" | "green" | "amber" | "purple" | "red" | "teal";
  size?: "sm" | "md" | "lg" | "xl";
}) => {
  const bgColors = {
    default: "bg-slate-800",
    blue: "bg-blue-900",
    green: "bg-emerald-900",
    amber: "bg-amber-900",
    purple: "bg-purple-900",
    red: "bg-rose-900",
    teal: "bg-teal-900",
  };

  const sizes = {
    sm: "w-8 h-8 md:w-10 md:h-10",
    md: "w-10 h-10 md:w-12 md:h-12",
    lg: "w-12 h-12 md:w-14 md:h-14",
    xl: "w-16 h-16 md:w-20 md:h-20",
  };

  return (
    <div className={`${sizes[size]} ${bgColors[variant]} rounded-lg flex items-center justify-center shadow-lg`}>
      <span className="text-white font-bold">{children}</span>
    </div>
  );
};

// Componente de número para flujos
const StepNumber = ({ 
  number, 
  variant = "default" 
}: { 
  number: number; 
  variant?: "default" | "blue" | "green" | "amber" | "purple";
}) => {
  const bgColors = {
    default: "bg-slate-800",
    blue: "bg-blue-900",
    green: "bg-emerald-900",
    amber: "bg-amber-900",
    purple: "bg-purple-900",
  };

  return (
    <div className={`w-12 h-12 md:w-14 md:h-14 ${bgColors[variant]} rounded-full flex items-center justify-center shadow-lg`}>
      <span className="text-white font-bold text-lg md:text-xl">{number}</span>
    </div>
  );
};

// Sección 1: Portada
const PortadaSection = () => (
  <section className="min-h-[calc(100vh-60px)] md:min-h-screen p-4 md:p-12 flex flex-col bg-slate-50">
    {/* Header */}
    <div className="flex items-center justify-between mb-6 md:mb-12">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 md:w-14 md:h-14 bg-slate-800 rounded-lg flex items-center justify-center shadow-lg">
          <span className="text-white font-bold text-base md:text-xl">RR</span>
        </div>
        <div>
          <h1 className="text-lg md:text-xl font-semibold text-slate-800">RR Logistics</h1>
          <p className="text-slate-500 text-xs md:text-sm">Monitoreo para Expediciones</p>
        </div>
      </div>
    </div>

    {/* Título principal */}
    <div className="text-center mb-6 md:mb-12 max-w-4xl mx-auto flex-1 flex flex-col justify-center">
      <div className="inline-flex items-center gap-2 bg-slate-800 text-white px-3 py-1.5 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium mb-4 md:mb-6 mx-auto shadow-lg">
        <span className="w-5 h-5 bg-white/20 rounded-full flex items-center justify-center text-[10px] font-bold">◉</span>
        Monitoreo Satelital para Expediciones
      </div>
      <h2 className="text-2xl md:text-5xl font-bold text-slate-800 mb-4 md:mb-6 leading-tight">
        Que sus familias los acompañen, aunque no puedan ir
      </h2>
      <p className="text-base md:text-xl text-slate-600 max-w-2xl mx-auto">
        Plataforma de monitoreo en tiempo real para expediciones, senderismo, misiones y grupos de aventura.
      </p>
    </div>

    {/* Propuesta de valor dual */}
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 max-w-5xl mx-auto mb-4 md:mb-8">
      <div className="bg-white rounded-xl p-4 md:p-8 border border-slate-200 shadow-sm">
        <div className="flex items-center gap-3 mb-3 md:mb-4">
          <IndustrialIcon variant="red" size="md">
            <span className="text-lg md:text-xl">♥</span>
          </IndustrialIcon>
          <h3 className="text-base md:text-lg font-semibold text-slate-800">Para las Familias</h3>
        </div>
        <p className="text-sm md:text-base text-slate-600 leading-relaxed">
          Tranquilidad de saber dónde están en todo momento. Un link privado 
          donde pueden ver la ubicación del grupo sin depender de llamadas o mensajes.
        </p>
      </div>

      <div className="bg-white rounded-xl p-4 md:p-8 border border-slate-200 shadow-sm">
        <div className="flex items-center gap-3 mb-3 md:mb-4">
          <IndustrialIcon variant="blue" size="md">
            <span className="text-lg md:text-xl">★</span>
          </IndustrialIcon>
          <h3 className="text-base md:text-lg font-semibold text-slate-800">Para tu Empresa</h3>
        </div>
        <p className="text-sm md:text-base text-slate-600 leading-relaxed">
          Un valor agregado que te diferencia de la competencia. Mayor confianza 
          de los participantes y sus seres queridos al saber su ubicación en todo momento.
        </p>
      </div>
    </div>

    {/* Footer de página - solo desktop */}
    <div className="hidden md:block mt-auto pt-4 border-t border-slate-200 text-center text-sm text-slate-400">
      <p>RR Logistics — Monterrey, Nuevo León, México — Página 1 de 5</p>
    </div>
  </section>
);

// Sección 2: Casos de Uso
const CasosUsoSection = () => {
  const casos = [
    { 
      letra: "M", 
      titulo: "Misiones de Iglesias", 
      desc: "Grupos de voluntarios van a comunidades remotas. Sus familias pueden ver en tiempo real cómo avanza el viaje.",
      valor: "Mayor tranquilidad para participantes",
      variant: "purple" as const
    },
    { 
      letra: "S", 
      titulo: "Senderismo", 
      desc: "Matacanes, La Huasteca, escalada. Rutas donde no hay señal por horas pero sí hay familias esperando.",
      valor: "Tour premium diferenciado",
      variant: "green" as const
    },
    { 
      letra: "C", 
      titulo: "Campamentos", 
      desc: "Campamentos de verano, retiros espirituales, programas scout. Las familias saben que el grupo llegó bien.",
      valor: "Mayor inscripción",
      variant: "amber" as const
    },
    { 
      letra: "T", 
      titulo: "Tours de Aventura", 
      desc: "Expediciones guiadas, turismo de naturaleza. Un servicio premium para clientes exigentes.",
      valor: "Cobro adicional",
      variant: "blue" as const
    },
    { 
      letra: "V", 
      titulo: "Viajes de Grupos", 
      desc: "Excursiones, tours organizados, viajes corporativos a zonas sin señal.",
      valor: "Protocolos de seguridad",
      variant: "teal" as const
    },
    { 
      letra: "B", 
      titulo: "Voluntariado", 
      desc: "Brigadas de ayuda, reforestación, investigación de campo.",
      valor: "Monitoreo sin señal",
      variant: "default" as const
    },
  ];

  return (
    <section className="min-h-[calc(100vh-60px)] md:min-h-screen p-4 md:p-12 flex flex-col bg-white">
      <div className="flex items-center gap-3 mb-4 md:mb-8">
        <IndustrialIcon size="sm">
          <span className="text-sm md:text-base">◆</span>
        </IndustrialIcon>
        <h2 className="text-xl md:text-3xl font-bold text-slate-800">¿Para Quién Es?</h2>
      </div>

      <p className="text-sm md:text-base text-slate-600 mb-4 md:mb-8 max-w-3xl">
        Diseñado para empresas y organizaciones que llevan grupos a zonas sin señal celular 
        y quieren ofrecer tranquilidad a los familiares.
      </p>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 flex-1 overflow-y-auto">
        {casos.map((caso) => (
          <div key={caso.letra} className="bg-slate-50 rounded-xl p-4 md:p-6 border border-slate-200">
            <div className="flex items-center gap-3 mb-2 md:mb-4">
              <IndustrialIcon variant={caso.variant} size="md">
                <span className="text-lg md:text-xl">{caso.letra}</span>
              </IndustrialIcon>
              <h3 className="text-base md:text-lg font-semibold text-slate-800">{caso.titulo}</h3>
            </div>
            <p className="text-xs md:text-sm text-slate-600 mb-2 md:mb-4">
              {caso.desc}
            </p>
            <div className="text-xs text-slate-500 bg-white rounded px-2 py-1 md:px-3 md:py-2 border border-slate-100">
              <strong>Valor:</strong> {caso.valor}
            </div>
          </div>
        ))}
      </div>

      <div className="hidden md:block mt-6 text-center text-sm text-slate-400">
        <p>Página 2 de 5</p>
      </div>
    </section>
  );
};

// Sección 3: Cómo Funciona
const ComoFuncionaSection = () => {
  const funcionalidades = [
    { 
      letra: "M", 
      titulo: "Mapa en Tiempo Real", 
      desc: "Ubicación actual del grupo sobre un mapa interactivo.",
      puntos: ["Posición actualizada cada pocos minutos", "Zoom para ver detalle del terreno"],
      variant: "blue" as const
    },
    { 
      letra: "H", 
      titulo: "Historial de Ruta", 
      desc: "Trazo completo del recorrido que ha hecho el grupo.",
      puntos: ["Línea de recorrido sobre el mapa", "Marcadores de paradas importantes"],
      variant: "green" as const
    },
    { 
      letra: "C", 
      titulo: "Checkpoints", 
      desc: "Notificaciones cuando el grupo llega a puntos importantes.",
      puntos: ['"El grupo salió a las 6:00 AM"', '"Llegaron al campamento base"'],
      variant: "amber" as const
    },
    { 
      letra: "A", 
      titulo: "Acceso Controlado", 
      desc: "Link privado o credenciales para los familiares autorizados.",
      puntos: ["Acceso individual por familia", "Privacidad garantizada"],
      variant: "default" as const
    },
  ];

  return (
    <section className="min-h-[calc(100vh-60px)] md:min-h-screen p-4 md:p-12 flex flex-col bg-slate-50">
      <div className="flex items-center gap-3 mb-4 md:mb-8">
        <IndustrialIcon size="sm">
          <span className="text-sm md:text-base">▶</span>
        </IndustrialIcon>
        <h2 className="text-xl md:text-3xl font-bold text-slate-800">¿Cómo Funciona?</h2>
      </div>

      <p className="text-sm md:text-base text-slate-600 mb-4 md:mb-8 max-w-3xl">
        Un proceso simple: el dispositivo va con el guía, transmite vía satélite, 
        y los familiares pueden ver la ubicación desde cualquier dispositivo.
      </p>

      {/* Flujo visual con números */}
      <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4 mb-6 md:mb-12">
        <div className="bg-white rounded-xl p-4 md:p-6 border border-slate-200 text-center flex-1 shadow-sm">
          <StepNumber number={1} variant="blue" />
          <div className="mt-3 md:mt-4">
            <h4 className="font-semibold text-slate-800 mb-1 md:mb-2">El Guía</h4>
            <p className="text-xs md:text-sm text-slate-600">
              Porta el dispositivo en mochila o vehículo
            </p>
          </div>
        </div>

        <div className="flex items-center justify-center">
          <div className="w-8 h-8 bg-slate-300 rounded-full flex items-center justify-center rotate-90 md:rotate-0">
            <span className="text-slate-600 font-bold">→</span>
          </div>
        </div>

        <div className="bg-white rounded-xl p-4 md:p-6 border border-slate-200 text-center flex-1 shadow-sm">
          <StepNumber number={2} variant="purple" />
          <div className="mt-3 md:mt-4">
            <h4 className="font-semibold text-slate-800 mb-1 md:mb-2">Satélite</h4>
            <p className="text-xs md:text-sm text-slate-600">
              Transmisión directa sin señal celular
            </p>
          </div>
        </div>

        <div className="flex items-center justify-center">
          <div className="w-8 h-8 bg-slate-300 rounded-full flex items-center justify-center rotate-90 md:rotate-0">
            <span className="text-slate-600 font-bold">→</span>
          </div>
        </div>

        <div className="bg-white rounded-xl p-4 md:p-6 border border-slate-200 text-center flex-1 shadow-sm">
          <StepNumber number={3} variant="green" />
          <div className="mt-3 md:mt-4">
            <h4 className="font-semibold text-slate-800 mb-1 md:mb-2">Familias</h4>
            <p className="text-xs md:text-sm text-slate-600">
              Ven la ubicación en tiempo real
            </p>
          </div>
        </div>
      </div>

      {/* Funcionalidades de la plataforma */}
      <h3 className="text-lg md:text-xl font-semibold text-slate-800 mb-4 md:mb-6">Lo que Ven los Familiares</h3>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 flex-1 overflow-y-auto">
        {funcionalidades.map((func) => (
          <div key={func.letra} className="bg-white rounded-xl p-4 md:p-6 border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 mb-3 md:mb-4">
              <IndustrialIcon variant={func.variant} size="sm">
                <span className="text-sm md:text-base">{func.letra}</span>
              </IndustrialIcon>
              <h4 className="text-base md:text-lg font-semibold text-slate-800">{func.titulo}</h4>
            </div>
            <p className="text-xs md:text-sm text-slate-600 mb-3 md:mb-4">
              {func.desc}
            </p>
            <ul className="space-y-1 md:space-y-2 text-xs md:text-sm text-slate-600">
              {func.puntos.map((punto, i) => (
                <li key={i} className="flex items-center gap-2">
                  <span className="w-4 h-4 bg-slate-800 rounded text-white text-[10px] flex items-center justify-center shrink-0">✓</span>
                  {punto}
                </li>
              ))}
            </ul>
          </div>
        ))}
      </div>

      <div className="hidden md:block mt-6 text-center text-sm text-slate-400">
        <p>Página 3 de 5</p>
      </div>
    </section>
  );
};

// Sección 4: Beneficios
const BeneficiosSection = () => {
  const beneficios = [
    { 
      letra: "D", 
      titulo: "Diferenciación", 
      desc: "Ofreces algo que otros no tienen. Tu servicio de monitoreo puede ser el factor decisivo.",
      variant: "blue" as const
    },
    { 
      letra: "C", 
      titulo: "Mayor Confianza", 
      desc: "Mayor conversión en reservaciones. Los participantes que antes dudaban, ahora se animan.",
      variant: "green" as const
    },
    { 
      letra: "I", 
      titulo: "Ingreso Adicional", 
      desc: "Ofrécelo como servicio premium opcional o inclúyelo en paquetes de mayor valor.",
      variant: "amber" as const
    },
    { 
      letra: "R", 
      titulo: "Respaldo Legal", 
      desc: "Historial completo de la ruta como evidencia. Tienes documentado todo el recorrido.",
      variant: "purple" as const
    },
  ];

  return (
    <section className="min-h-[calc(100vh-60px)] md:min-h-screen p-4 md:p-12 flex flex-col bg-white">
      <div className="flex items-center gap-3 mb-4 md:mb-8">
        <IndustrialIcon size="sm">
          <span className="text-sm md:text-base">★</span>
        </IndustrialIcon>
        <h2 className="text-xl md:text-3xl font-bold text-slate-800">Beneficios para tu Empresa</h2>
      </div>

      <p className="text-sm md:text-base text-slate-600 mb-4 md:mb-8 max-w-3xl">
        No es solo un servicio para los familiares. Es una herramienta estratégica 
        que mejora tu operación y te posiciona como líder.
      </p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 mb-4 md:mb-8">
        {beneficios.map((ben) => (
          <div key={ben.letra} className="bg-slate-50 rounded-xl p-4 md:p-6 border border-slate-200">
            <div className="flex items-center gap-3 mb-3 md:mb-4">
              <IndustrialIcon variant={ben.variant} size="sm">
                <span className="text-sm md:text-base">{ben.letra}</span>
              </IndustrialIcon>
              <h4 className="text-base md:text-lg font-semibold text-slate-800">{ben.titulo}</h4>
            </div>
            <p className="text-xs md:text-sm text-slate-600">
              {ben.desc}
            </p>
          </div>
        ))}
      </div>

      {/* Tabla comparativa */}
      <h3 className="text-lg md:text-xl font-semibold text-slate-800 mb-3 md:mb-4">Comparativa</h3>
      
      <div className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden flex-1 overflow-y-auto">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[400px]">
            <thead>
              <tr className="bg-slate-800 text-white">
                <th className="text-left p-2 md:p-4 font-semibold text-xs md:text-sm">Situación</th>
                <th className="text-left p-2 md:p-4 font-semibold text-xs md:text-sm">Sin Monitoreo</th>
                <th className="text-left p-2 md:p-4 font-semibold text-xs md:text-sm">Con RR Logistics</th>
              </tr>
            </thead>
            <tbody className="text-xs md:text-sm">
              <tr className="border-t border-slate-200">
                <td className="p-2 md:p-4 text-slate-700 font-medium">Misión con 30 personas</td>
                <td className="p-2 md:p-4 text-slate-600">Familias 3 días sin noticias</td>
                <td className="p-2 md:p-4 text-slate-600">Ubicación visible siempre</td>
              </tr>
              <tr className="border-t border-slate-200 bg-white">
                <td className="p-2 md:p-4 text-slate-700 font-medium">Senderismo en Matacanes</td>
                <td className="p-2 md:p-4 text-slate-600">Sin señal 8+ horas</td>
                <td className="p-2 md:p-4 text-slate-600">Posición vía satélite</td>
              </tr>
              <tr className="border-t border-slate-200">
                <td className="p-2 md:p-4 text-slate-700 font-medium">Campamento en la sierra</td>
                <td className="p-2 md:p-4 text-slate-600">Incertidumbre total</td>
                <td className="p-2 md:p-4 text-slate-600">Checkpoints automáticos</td>
              </tr>
            </tbody>
          </table>
        </div>
      </div>

      <div className="hidden md:block mt-6 text-center text-sm text-slate-400">
        <p>Página 4 de 5</p>
      </div>
    </section>
  );
};

// Sección 5: Contacto
const ContactoSection = () => (
  <section className="min-h-[calc(100vh-60px)] md:min-h-screen p-4 md:p-12 flex flex-col bg-slate-50">
    <div className="flex-1 flex flex-col items-center justify-center text-center max-w-3xl mx-auto">
      {/* Logo grande */}
      <div className="w-16 h-16 md:w-24 md:h-24 bg-slate-800 rounded-2xl flex items-center justify-center mb-4 md:mb-8 shadow-xl">
        <span className="text-white font-bold text-2xl md:text-4xl">RR</span>
      </div>

      <h2 className="text-xl md:text-4xl font-bold text-slate-800 mb-3 md:mb-4">
        ¿Organizas expediciones?
      </h2>
      
      <p className="text-sm md:text-xl text-slate-600 mb-6 md:mb-8">
        Ofrece a los familiares la tranquilidad que merecen. 
        Conoce nuestra plataforma de monitoreo.
      </p>

      {/* Beneficios resumidos con estilo industrial */}
      <div className="grid grid-cols-3 gap-3 md:gap-6 mb-6 md:mb-12 w-full">
        <div className="text-center">
          <div className="w-10 h-10 md:w-12 md:h-12 bg-blue-900 rounded-lg flex items-center justify-center mx-auto mb-2 md:mb-3 shadow-lg">
            <span className="text-white font-bold text-sm md:text-base">◉</span>
          </div>
          <p className="text-xs md:text-sm text-slate-600">Sin señal celular</p>
        </div>
        <div className="text-center">
          <div className="w-10 h-10 md:w-12 md:h-12 bg-emerald-900 rounded-lg flex items-center justify-center mx-auto mb-2 md:mb-3 shadow-lg">
            <span className="text-white font-bold text-sm md:text-base">◎</span>
          </div>
          <p className="text-xs md:text-sm text-slate-600">Tiempo real</p>
        </div>
        <div className="text-center">
          <div className="w-10 h-10 md:w-12 md:h-12 bg-purple-900 rounded-lg flex items-center justify-center mx-auto mb-2 md:mb-3 shadow-lg">
            <span className="text-white font-bold text-sm md:text-base">◈</span>
          </div>
          <p className="text-xs md:text-sm text-slate-600">Acceso seguro</p>
        </div>
      </div>

      {/* Información de contacto */}
      <div className="bg-white rounded-xl p-4 md:p-8 border border-slate-200 w-full shadow-sm">
        <h3 className="text-base md:text-lg font-semibold text-slate-800 mb-4 md:mb-6">
          Solicita una demostración
        </h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          <div className="flex items-center gap-3 md:gap-4">
            <IndustrialIcon variant="blue" size="md">
              <span className="text-lg">☎</span>
            </IndustrialIcon>
            <div className="text-left">
              <p className="text-xs md:text-sm text-slate-500">Teléfonos</p>
              <p className="font-semibold text-slate-800 text-sm md:text-base">81-1016-6812</p>
              <p className="font-semibold text-slate-800 text-sm md:text-base">311-122-3365</p>
            </div>
          </div>
          
          <div className="flex items-center gap-3 md:gap-4">
            <IndustrialIcon size="md">
              <span className="text-lg">⌂</span>
            </IndustrialIcon>
            <div className="text-left">
              <p className="text-xs md:text-sm text-slate-500">Ubicación</p>
              <p className="font-semibold text-slate-800 text-sm md:text-base">Monterrey, Nuevo León</p>
              <p className="text-xs md:text-sm text-slate-500">México</p>
            </div>
          </div>
        </div>
      </div>

      {/* CTA final */}
      <div className="mt-4 md:mt-8 p-4 md:p-6 bg-slate-800 rounded-xl text-white w-full shadow-lg">
        <p className="text-sm md:text-lg font-medium">
          "Que sus familias los acompañen, aunque no puedan ir"
        </p>
      </div>
    </div>

    {/* Footer */}
    <div className="hidden md:block mt-8 pt-4 border-t border-slate-200 text-center text-sm text-slate-400">
      <p>RR Logistics — Tecnología de Monitoreo para Expediciones — Página 5 de 5</p>
    </div>
  </section>
);

const FichaExpediciones = () => {
  const [activeTab, setActiveTab] = useState(0);

  const sections = [
    <PortadaSection key="portada" />,
    <CasosUsoSection key="casos" />,
    <ComoFuncionaSection key="como" />,
    <BeneficiosSection key="beneficios" />,
    <ContactoSection key="contacto" />,
  ];

  return (
    <div className="min-h-screen bg-white">
      {/* Tab Navigation - Solo móvil */}
      <nav className="md:hidden sticky top-0 z-50 bg-white border-b border-slate-200 shadow-sm">
        <div className="flex overflow-x-auto scrollbar-hide">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex-1 min-w-[70px] px-3 py-3 text-xs font-medium transition-colors whitespace-nowrap ${
                activeTab === tab.id
                  ? "text-slate-800 border-b-2 border-slate-800 bg-slate-50"
                  : "text-slate-500 hover:text-slate-700 hover:bg-slate-50"
              }`}
            >
              {tab.short}
            </button>
          ))}
        </div>
        {/* Progress indicator */}
        <div className="h-0.5 bg-slate-100">
          <div 
            className="h-full bg-slate-800 transition-all duration-300"
            style={{ width: `${((activeTab + 1) / tabs.length) * 100}%` }}
          />
        </div>
      </nav>

      {/* Contenido móvil - Solo la sección activa */}
      <div className="md:hidden">
        {sections[activeTab]}
      </div>

      {/* Contenido desktop - Todas las secciones con scroll */}
      <div className="hidden md:block">
        {sections}
      </div>
    </div>
  );
};

export default FichaExpediciones;
